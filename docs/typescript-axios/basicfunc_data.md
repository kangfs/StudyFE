...
这个时候 `data`是不能直接传给 `send` 函数的，我们需要把它转换成 JSON 字符串。而字符串是属于上面的`USVString`类型的,**而其他`Blob`, `BufferSource`, `FormData`, `URLSearchParams`, `ReadableStream`这些类型本身就支持，不需要做转化,我们只需要对普通的对象做转化**。
