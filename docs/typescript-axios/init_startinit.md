...
接下来，我们开始初始化项目，首先我们先去 GitHub 上创建一个 repo，填好 repo 名称(我这里填的是tzc-axios)，以及写一下 README，对项目先做个简单的描述。
...
```bash
git clone https://github.com/alexjoverm/typescript-library-starter.git tzc-axios
cd ts-axios

npm install
```

先通过 `git clone` 把项目代码拉下来到我们的 `tzc-axios` 目录，然后运行 `npm install` 安装依赖，并且给项目命名，我们仍然使用 `tzc-axios`。
...
```bash
git remote add origin 仓库地址(我这里是git@github.com:taopoppy/tzc-axios.git)
```
