...
- 首先判断如果是配置 `withCredentials` 为 `true`(`withCredentials`为`true`表示跨域中可以携带cookie) 或者是同域请求(同域请求会自带cookie)，我们才会请求 `headers` 添加 `xsrf` 相关的字段。
...
至此，我们实现了 XSRF 的自动防御的能力，下节课我们来实现 tzc-axios 对上传和下载请求的支持。
