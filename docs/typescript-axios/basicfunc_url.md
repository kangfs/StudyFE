...
export function buildURL(url: string, params?: any): string {
  ...
  Object.keys(params).forEach((key) => {
    ...
    if(val === null || val === undefined) {       // 空值忽略
      return
    }
    let values = []
    if(Array.isArray(val)) {                      // 参数值为数组
      values = val
      key += '[]'
    } else {
      values = [val]
    ...
    values.forEach((val) => {
      if(isData(val)) {                           // 参数值为Date类型
        val = val.toISOString()
      } else if(isObject(val)) {                  // 参数值为对象类型
        val = JSON.stringify(val)
      }
      parts.push(`${encode(key)}=${encode(val)}`) // encode和特殊字符处理
    })
  })
...
然后在命令行运行 `npm run dev`，接着打开 chrome 浏览器，访问 `http://localhost:8090/` 即可访问我们的 demo 了，我们点到 `Base` 目录下，通过开发者工具的 network 部分我们可以看到成功发送的多条请求，并可以观察它们最终请求的 url，已经如期添加了请求参数。
